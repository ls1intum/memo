...
public interface CompetencyResourceLinkRepository extends JpaRepository<CompetencyResourceLink, String> {
  ...
  @Query(value = "SELECT COUNT(*) FROM competency_resource_links WHERE competency_id = :id", nativeQuery = true)
  long countByCompetencyId(@Param("id") String competencyId);

  @Query(value = """
      SELECT COALESCE(AVG(CASE match_type
      ...
      """, nativeQuery = true)
  double averageMatchQualityByCompetencyId(@Param("id") String competencyId);
}
