...
import type { RelationshipImportRow, ImportResult } from '@/lib/api/types';

type Mode = 'file' | 'json';
type Status = 'idle' | 'uploading' | 'success' | 'error';

const PLACEHOLDER = `[
  { "originId": "...", "destinationId": "...", "relationshipType": "ASSUMES" },
  { "originTitle": "Big-O notation", "destinationTitle": "Sorting algorithms" }
]`;

export function RelationshipImportPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'file') {
        ...
        importResult = await adminApi.importRelationshipsFile(selectedFile);
      } else {
        let rows: RelationshipImportRow[];
        try {
          rows = JSON.parse(jsonText) as RelationshipImportRow[];
        } catch {
          setJsonError('Invalid JSON. Please check the format.');
          setStatus('idle');
          return;
        }
        importResult = await adminApi.importRelationshipsJson(rows);
      }
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-slate-200/60 bg-slate-50/60 p-4">
                    ...
                    <pre className="whitespace-pre-wrap font-mono text-xs text-slate-600">
                      {`originId,originTitle,destinationId,destinationTitle,relationshipType\n"uuid-1",,"uuid-2",,"ASSUMES"\n,"Big-O notation",,"Sorting algorithms",`}
                    </pre>
                  </div>
                </div>
              ) : (
