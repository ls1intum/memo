    ...
    <div className="relative min-h-screen overflow-hidden bg-linear-to-br from-[#d7e3ff] via-[#f3f5ff] to-[#e8ecff] text-slate-900 dark:from-[#0f1729] dark:via-[#111b30] dark:to-[#0f1729] dark:text-slate-100">
      ...
      <main className="relative z-10 mx-auto mt-20 w-full max-w-4xl px-6 pb-24 lg:mt-24 lg:px-0">
        ...
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          ...
          {data.length > 0 && (
            <span className="rounded-full border border-slate-200/80 bg-white/80 px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm dark:border-slate-700/50 dark:bg-slate-800/60 dark:text-slate-300">
              {data.length} total
            </span>
          )}
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                {data.map((u: User) => (
                  ...
                  >
                    <td className="py-3 pr-6 font-mono text-xs text-slate-400 dark:text-slate-500">
                      {anonId(u.id)}
                    </td>
                    <td className="py-3 pr-6">
                      {u.role === 'ADMIN' ? (
                        <span className="inline-flex items-center rounded-full bg-[#0a4da2]/10 px-2.5 py-0.5 text-xs font-semibold text-[#0a4da2] dark:bg-[#0a4da2]/20 dark:text-[#6b9fff]">
                          Admin
                        </span>
                      ) : (
                        <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-500 dark:bg-slate-700 dark:text-slate-300">
                          User
                        </span>
                      )}
                    </td>
                    <td className="py-3 pr-6 text-xs text-slate-400 hidden sm:table-cell dark:text-slate-500">
                    ...
                    <td className="py-3 text-right">
                      <button
                        ...
                        className="rounded-full border border-slate-200/80 bg-white px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-[#0a4da2]/40 hover:text-[#0a4da2] disabled:opacity-50 dark:border-slate-600 dark:bg-slate-800/80 dark:text-slate-300 dark:hover:border-[#6b9fff]/40 dark:hover:text-[#6b9fff]"
                      >
